    ...
    for(int i = 0; i < threadsNum; ++i)
    {
        threads[i] = std::thread([&, i, facesNum, threadFacesNum]()
        {
            for(size_t j = i * threadFacesNum; j < (i + 1) * threadFacesNum; ++j)
            {
                ...
                glm::vec3 tangent = firstVert - secondVert;
                glm::vec3 bitangent = firstVert - thirdVert;
                ...
                if(backFaceCulling && glm::dot(normal, -firstVert) > 0)
                        continue;              
                ...
                if(rasterisation)
                {
                    drawTriangle(firstVert, secondVert, thirdVert, center.z, colorUI);
                }
                if(wireframe)
                {
